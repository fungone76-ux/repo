...
class TurnOrchestrator:
        ...
        if movement_result.companion_left_behind and game_state.active_companion:
            ...
            if self.schedule_manager:
                ...
                if schedule_companion and schedule_companion != solo_name:
                    print(f"[TurnOrchestrator] Auto-switch on arrival: solo -> {schedule_companion}")
                    self.state_manager.switch_companion(schedule_companion)
                    self.engine.companion = schedule_companion
                    solo_name = schedule_companion
    ...
    def _validate_updates(self, updates: Any, game_state: GameState) -> Dict[str, Any]:
        ...
        # Validate affinity changes
        if hasattr(updates, 'affinity_change') and updates.affinity_change:
            result['affinity_change'] = {}
            for npc, delta in updates.affinity_change.items():
                if isinstance(delta, (int, float)):
                    # Clamp to reasonable range
                    result['affinity_change'][npc] = max(-50, min(50, delta))
        
        # Validate outfit changes
        if hasattr(updates, 'outfit_change') and updates.outfit_change:
            result['outfit_change'] = updates.outfit_change
        
        # Validate new fact
        if hasattr(updates, 'new_fact') and updates.new_fact:
            result['new_fact'] = updates.new_fact[:500]  # Limit length
        
        # Validate photo request
        if hasattr(updates, 'photo_requested'):
    ...
    def _apply_updates(self, validated_updates: Dict[str, Any], game_state: GameState) -> None:
        ...
        if 'affinity_change' in validated_updates:
            for npc, delta in validated_updates['affinity_change'].items():
                if npc in game_state.npc_affinity:
                    game_state.npc_affinity[npc] += delta
                    # Clamp
                    game_state.npc_affinity[npc] = max(0, min(100, game_state.npc_affinity[npc]))
    ...
    def _handle_phase_manager(self) -> Optional[Any]:
        ...
        with self.tracer.step_context("Phase Manager", "phase"):
            ...
            if phase_result:
                from luna.core.debug_tracing import CheckStatus
    ...
    async def _generate_media(
        ...
        if multi_npc_sequence:
            ...
            media_result = await self._generate_standard_media(game_state, llm_response)
    ...
    async def _generate_standard_media(
        ...
        if self._no_media:
            with self.tracer.step_context("Media Prompt", "media"):
                ...
                self.tracer.info(f"Location: {location_id} - {location_desc}")
            return None
