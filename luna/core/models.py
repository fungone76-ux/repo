...
class OutfitModification(LunaBaseModel):
    """Single overlay modification applied on top of the base outfit.
    
    Modifications are stored in OutfitState.modifications and reset
    when the phase changes or when the player explicitly removes them.
    """
    
    component: str = Field(description="Component being modified: shoes, top, bottom, etc.")
    """Clothing component this modification affects."""
    
    state: str = Field(description="Modification state: removed, wet, torn, lifted, etc.")
    """How the component is modified."""
    
    description: str = Field(default="")
    """Italian description of the modification (for LLM context)."""
    
    sd_description: str = Field(default="")
    """English SD prompt description of the modification (for image generation)."""
    
    applied_at_turn: int = Field(default=0, ge=0)
    """Turn when this modification was applied."""


class OutfitState(LunaBaseModel):
    """Detailed outfit state for a character.
    
    V5.0: Enhanced with base/LLM-generated outfit support and overlay modifications.
    ...
    """Wardrobe style key (casual, formal, sexy_custom, etc.)."""
    
    description: str = Field(default="")
    """Free-form description (legacy / backward-compat). Prefer base_description."""
    ...
    # -------------------------------------------------------------------------
    # V5.0: Base outfit (set when outfit changes via schedule or explicit change)
    # -------------------------------------------------------------------------
    
    base_description: str = Field(default="")
    """Base outfit description from wardrobe (Italian, human-readable)."""
    
    base_sd_prompt: str = Field(default="")
    """Base SD prompt from wardrobe (English, for image generation)."""
    
    # -------------------------------------------------------------------------
    # V5.0: LLM-generated custom outfit (created once, persisted until change)
    # -------------------------------------------------------------------------
    
    llm_generated_description: Optional[str] = Field(default=None)
    """Custom outfit description generated by LLM (Italian). Persists until explicit change."""
    
    llm_generated_sd_prompt: Optional[str] = Field(default=None)
    """Custom outfit SD prompt generated by LLM (English). Persists until explicit change."""
    
    generated_at_turn: int = Field(default=0, ge=0)
    """Turn when the LLM-generated outfit was created."""
    
    # -------------------------------------------------------------------------
    # V5.0: Overlay modifications (reset at phase change)
    # -------------------------------------------------------------------------
    
    modifications: Dict[str, OutfitModification] = Field(default_factory=dict)
    """Active overlay modifications keyed by component: {"shoes": OutfitModification, ...}.
    
    These are reset when the phase changes or on explicit player request.
    """
    ...
    def to_prompt_string(self) -> str:
        ...
        Priority:
        1. LLM-generated description (custom outfit)
        2. base_description (from wardrobe)
        3. Legacy description field
        
        Modifications are appended as overlay.
        """
        if self.is_special:
            return f"[Special: {self.description}]"
        
        # Determine base
        if self.llm_generated_description:
            base = self.llm_generated_description
        elif self.base_description:
            base = self.base_description
        elif self.description:
            base = self.description
        else:
            return f"wearing {self.style} clothes"
        
        # Append active modifications
        if self.modifications:
            mod_descs = [m.description for m in self.modifications.values() if m.description]
            if mod_descs:
                return f"{base}, {', '.join(mod_descs)}"
